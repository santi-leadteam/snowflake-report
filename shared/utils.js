...
const firstOfThisMonth = new Date(__now.getFullYear(), __now.getMonth(), 1);
const firstOfPrevMonth = new Date(__now.getFullYear(), __now.getMonth() - 1, 1);
...
if (fFromEl) fFromEl.value = localDateStr(firstOfPrevMonth);
if (fToEl)   fToEl.value   = localDateStr(lastOfPrevMonth);
...
function updateLabel(menuId, labelId, items) {
  ...
  const checked = [...menu.querySelectorAll('input[value]:checked')].map(c => c.value);
...
function getRowDate(r) {
  return new Date((r.date || r.month) + 'T00:00:00Z');
}
...
function sumRows(rows) {
  return rows.reduce((acc, r) => {
    acc.leads += r.signups || 0;
    acc.ft += r.first_visits || 0;
    acc.mem += r.first_activations || 0;
    return acc;
  }, { leads:0, ft:0, mem:0 });
}
...
function computeWindows(from, to) {
  ...
  const pyFrom = new Date(from);
  pyFrom.setFullYear(pyFrom.getFullYear() - 1);
  const pyTo   = new Date(to);
...
function fmtMonthLabel(iso) {
  return new Date(iso+'T00:00:00Z').toLocaleString('en-US', { month:'short', year:'2-digit', timeZone:'UTC' });
}

function fmtDayLabel(iso) {
  return new Date(iso+'T00:00:00Z').toLocaleString('en-US', { month:'short', day:'numeric', timeZone:'UTC' });
}
...
function toTimeSeries(dailyRows, monthlyRows) {
  if (GRAN === 'daily') {
    ...
    dailyRows.forEach(r => {
      ...
      map[r.date].signups += r.signups || 0;
      map[r.date].first_visits += r.first_visits || 0;
      map[r.date].first_activations += r.first_activations || 0;
    });
  ...
  if (GRAN === 'weekly') {
    ...
    [...dailyRows, ...monthlyRows].forEach(r => {
      ...
      const ws = new Date(d);
      ws.setUTCDate(d.getUTCDate() - day + 1);
      ...
      map[key].first_visits += r.first_visits || 0;
      map[key].first_activations += r.first_activations || 0;
    });
    return Object.values(map).sort((a,b) => a.key.localeCompare(b.key));
  }
...
function srcColor(src, i) {
  return SRC_COLOR_MAP[src] || SRC_COLORS[i % SRC_COLORS.length];
}
...
function buildAreaChart(canvasId, togglesId, series, srcList, valueKey) {
  ...
  if (_areaCharts[canvasId]) {
    _areaCharts[canvasId].destroy();
    _areaCharts[canvasId] = null;
  }
  ...
  const datasets = sortedSrcs.map(src => {
    ...
    return {
      label: src,
      data: timeKeys.map(tk => series.filter(r => r.key === tk && r.source === src).reduce((s, r) => s + (r[valueKey] || 0), 0)),
      borderColor: c,
      ...
      plugins:{
        ...
        tooltip:{
          ...
          callbacks:{
            ...
            afterBody: items => {
              const t = items.filter(i => !i.dataset.hidden).reduce((s, i) => s + i.parsed.y, 0);
  ...
  if (tg) {
    ...
    sortedSrcs.forEach((src, btnI) => {
      ...
      btn.onclick = function() {
        ...
        if (!ch) return;
        const ds = ch.data.datasets[parseInt(this.dataset.dsIdx, 10)];
        ds.hidden = !ds.hidden;
        ...
        if (this.classList.contains('on')) {
          ...
          this.style.background = '#f4f9fd';
          this.style.borderColor = '#c0d4df';
...
function buildRingChart(canvasId, legendId, labels, values) {
  ...
  _areaCharts[canvasId] = new Chart(ctx, {
    type:'doughnut',
    data:{
      labels,
      datasets:[{
  ...
  if (leg) {
    leg.innerHTML = labels.map((l,i) => {
      ...
      return '<div class="ring-legend-item"><span class="ring-legend-dot" style="background:'+colors[i]+'"></span><span>'+l+'</span><span style="margin-left:auto;font-weight:600;color:#1a1a2e">'+pct+'%</span></div>';
    }).join('');
  }
}
...
function buildStudioRankTable(tableId, rows, valueKey) {
  ...
  const byS = {};
  rows.forEach(r => { byS[r.studio] = (byS[r.studio] || 0) + (r[valueKey] || 0); });
  const sorted = Object.entries(byS).sort((a,b) => b[1] - a[1]);
  const total = sorted.reduce((s,[,v]) => s + v, 0);

  tbody.innerHTML = sorted.map(([studio,val]) => {
    const pct = total ? (val / total * 100).toFixed(1) : 0;
    const bw = total ? Math.round(val / total * 80) : 0;
    return '<tr><td><span class="bar-mini" style="width:'+bw+'px"></span>'+studio.replace('SWEAT440 ','')+'</td><td class="num">'+val.toLocaleString()+'</td><td class="pct">'+pct+'%</td></tr>';
  }).join('');
}
...
function toSourceTimeSeries(dailyRows, monthlyRows, gran) {
  ...
  allRows.forEach(r => {
    ...
    if (g === 'daily') {
      ...
      key = (r.date || r.month).slice(0,7) + '-01';
    }

    const mk = key + '|' + r.source;
    if (!map[mk]) map[mk] = { key, source:r.source, signups:0, first_visits:0, first_activations:0 };
    map[mk].signups += r.signups || 0;
    map[mk].first_visits += r.first_visits || 0;
    map[mk].first_activations += r.first_activations || 0;
  });
...
function getMock() {
  ...
  function seed(studio, source, month) {
    const si = studios.indexOf(studio);
    const ri = sources.indexOf(source);
  ...
  const daily = [];
  const baseMs = new Date('2026-01-20T00:00:00Z').getTime();
  for (let i = 89; i >= 0; i--) {
